#!/bin/sh
# Create default namespace in Temporal (from temporalio/samples-server).
set -eu
...
temporal operator namespace describe -n $NAMESPACE --address $TEMPORAL_ADDRESS 2>/dev/null || temporal operator namespace create -n $NAMESPACE --address $TEMPORAL_ADDRESS
